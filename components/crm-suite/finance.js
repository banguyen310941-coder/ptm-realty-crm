...
function vietnamParts(){return Object.fromEntries(new Intl.DateTimeFormat("en-GB",{timeZone:VN_TZ,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}).formatToParts(new Date()).map(({type,value})=>[type,value]))}
...
function toLocalDateTime(value){if(!value)return"";const raw=String(value);if(!/[zZ]|[+-]\d\d:\d\d$/.test(raw))return raw.slice(0,16);const d=new Date(raw);if(Number.isNaN(d.getTime()))return raw.slice(0,16);const p=Object.fromEntries(new Intl.DateTimeFormat("en-GB",{timeZone:VN_TZ,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}).formatToParts(d).map(({type,value})=>[type,value]));return `${p.year}-${p.month}-${p.day}T${p.hour}:${p.minute}`}
function toVietnamIso(value){if(!value)return"";const raw=String(value);if(/[zZ]|[+-]\d\d:\d\d$/.test(raw))return raw;return `${raw.length===16?raw+":00":raw}+07:00`}
...
export function FinanceModule({ data, finance, financeReady, financeMutate }) {
  ...
  function openPayment(p={}){setModal({type:"payment",draft:{...EMPTY_PAYMENT,...p,paid_at:p.paid_at?toLocalDateTime(p.paid_at):""}})}
...
function DepositModal({modal,setModal,save}){
  ...
  const submit=async e=>{e.preventDefault();await save("workflow.deposit.confirm",{...d,paid_at:toVietnamIso(d.paid_at)});setModal(null)};
