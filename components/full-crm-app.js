...
import { SESSION_KEY, coreRpc, fullRpc, financeRpc, cemeteryRpc, loginRpc, automationEvent, automationSweep } from "@/lib/crm-client";
...
export default function FullCRMApp(){
  ...
  async function bulkImportLeads(rows,onProgress){let success=0;const failed=[];for(let i=0;i<rows.length;i++){const r=rows[i];try{const out=await coreRpc(session?.token,"save_lead",{name:r.name,phone:r.phone,email:r.email||"",source:r.source||"Khác",project:r.project||"Thiên Phúc Vĩnh Hằng Viên",need:r.need||"",budget:r.budget||0,status:r.status||"new",notes:r.notes||"",owner_id:""});success++;await automationEvent("lead_created",{lead_id:out.id}).catch(()=>{});await automationEvent("lead_assigned",{lead_id:out.id}).catch(()=>{})}catch(e){failed.push({row:r.row,error:e.message})}onProgress?.(i+1,rows.length)}await load(session?.token,true);return{success,failed}}
