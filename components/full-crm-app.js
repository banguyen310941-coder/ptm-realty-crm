...
import { SESSION_KEY, coreRpc, fullRpc, financeRpc, loginRpc } from "@/lib/crm-client";
...
const NAV=[
  ...
  {id:"marketing",icon:"◈",label:"Marketing",group:"Tăng trưởng"},{id:"tasks",icon:"✓",label:"Công việc",group:"Vận hành"},{id:"calendar",icon:"□",label:"Lịch hẹn",group:"Vận hành"},{id:"tickets",icon:"◉",label:"Ticket CSKH",group:"Vận hành"},
  {id:"properties",icon:"⌂",label:"Dự án & giỏ hàng",group:"Bất động sản"},{id:"deals",icon:"₫",label:"Giao dịch",group:"Bất động sản"},{id:"finance",icon:"▤",label:"Hợp đồng & Tài chính",group:"Bất động sản"},
...
export default function FullCRMApp(){
  ...
  async function coreMutate(action,payload={}){setBusy(true);setError("");try{await coreRpc(session?.token,action,payload);await load(session?.token,true)}catch(e){setError(e.message);throw e}finally{setBusy(false)}}
  async function fullMutate(action,payload={}){setBusy(true);setError("");try{await fullRpc(session?.token,action,payload);await load(session?.token,true)}catch(e){setError(e.message);throw e}finally{setBusy(false)}}
  async function financeMutate(action,payload={}){setBusy(true);setError("");try{await financeRpc(session?.token,action,payload);await load(session?.token,true)}catch(e){setError(e.message);throw e}finally{setBusy(false)}}
