...
import { TicketsModule, TasksModule, CalendarModule, KpiModule } from "@/components/crm-suite/operations";
import { TeamModule } from "@/components/crm-suite/team-employees";
...
import { FacebookInboxModule } from "@/components/crm-suite/facebook-inbox";
import { HelpCenter } from "@/components/crm-suite/help-center";
...
const NAV=[
  ...
  {id:"marketing",icon:"◈",label:"Marketing",group:"Tăng trưởng"},{id:"facebook",icon:"f",label:"Chat Fanpage",group:"Tăng trưởng"},{id:"import",icon:"⇧",label:"Nhập khách Excel",group:"Tăng trưởng"},
...
export default function FullCRMApp(){
  ...
  <main className="suite-main"><header className="suite-topbar"><div><span className="eyebrow">THIÊN PHÚC VĨNH HẰNG VIÊN</span><b>{NAV.find(n=>n.id===view)?.label}</b></div><div className="suite-top-actions"><button className="suite-icon-btn" onClick={()=>setView("help")} title="Cài app & Hướng dẫn">?</button><button className="suite-icon-btn" onClick={()=>load()} title="Làm mới">↻</button><div className="suite-notification"><button className="suite-icon-btn" onClick={()=>setNoticeOpen(!noticeOpen)}>♢{unread>0&&<em>{unread}</em>}</button>{noticeOpen&&<div className="suite-notification-pop"><header><b>Thông báo</b><span>{unread} chưa đọc</span></header>{(full.notifications||[]).length?(full.notifications||[]).slice(0,15).map(n=><button key={n.id} className={n.read_at?"":"unread"} onClick={()=>readNotification(n)}><b>{n.title}</b><span>{n.body||""}</span><small>{fmtDate(n.created_at,true)}</small></button>):<div className="suite-empty">Chưa có thông báo.</div>}</div>}</div><div className="suite-user-chip"><span>{data.user.name}</span><small>{roleLabel(data.user.role)}</small></div></div></header>
  ...
  <section className="suite-content">{view==="dashboard"&&<ExecutiveDashboard data={data} full={full} permissions={permissions} finance={finance} financeReady={financeReady}/>} {view==="customers"&&<CustomersModule data={data} full={full} permissions={permissions} coreMutate={coreMutate} fullMutate={fullMutate} finance={finance} financeReady={financeReady} cemeteryAction={cemeteryAction}/>} {view==="opportunities"&&<OpportunitiesModule data={data} full={full} permissions={permissions} fullMutate={fullMutate} cemeteryAction={cemeteryAction}/>} {view==="marketing"&&<MarketingModule data={data} full={full} fullMutate={fullMutate}/>} {view==="facebook"&&<FacebookInboxModule sessionToken={session?.token}/>} {view==="import"&&<ImportLeadsModule data={data} inventory={EMPTY_IMPORT_HISTORY} inventoryReady={false} inventoryMutate={async()=>({})} bulkImportLeads={bulkImportLeads}/>} {view==="tasks"&&<TasksModule data={data} coreMutate={coreMutate}/>} {view==="calendar"&&<CalendarModule data={data} full={full}/>} {view==="tickets"&&<TicketsModule data={data} full={full} fullMutate={fullMutate}/>} {view==="properties"&&<CemeteryInventoryModule cemeteryAction={cemeteryAction} permissions={permissions}/>} {view==="deals"&&<CemeteryDealsModule data={data} permissions={permissions} coreMutate={coreMutate} cemeteryAction={cemeteryAction} finance={finance}/>} {view==="finance"&&<FinanceModule data={data} finance={finance} financeReady={financeReady} financeMutate={financeMutate}/>} {view==="kpi"&&<KpiModule data={data} full={full} fullMutate={fullMutate}/>} {view==="reports"&&<ReportsModule data={data} full={full} permissions={permissions} finance={finance} financeReady={financeReady}/>} {view==="automation"&&<AutomationModule data={data} full={full} fullMutate={fullMutate}/>} {view==="team"&&<TeamModule data={data} permissions={permissions} coreMutate={coreMutate}/>} {view==="help"&&<HelpCenter data={data}/>}</section></main></div>;
}

function LoginScreen({login,setLogin,signIn,busy,error}){return <main className="suite-login"><section className="suite-login-brand"><div className="brand-mark">PTM</div><span>PHÚC TRƯỜNG MINH</span><h1>CRM điều hành<br/>Thiên Phúc Vĩnh Hằng Viên</h1><p>Một nền tảng cho Marketing · Sales · CSKH · Kế toán · Ban giám đốc</p><div className="suite-login-features"><b>Customer 360°</b><b>Pipeline & Automation</b><b>Hợp đồng & Công nợ</b><b>Giỏ 9.392 mộ phần</b></div></section><section className="suite-login-form"><form onSubmit={signIn}><span className="eyebrow">PTM ENTERPRISE CRM</span><h2>Đăng nhập hệ thống</h2><p>Sử dụng tài khoản nhân viên được cấp bởi công ty.</p><label>Email<input type="email" required autoFocus autoComplete="username" value={login.email} onChange={e=>setLogin({...login,email:e.target.value})}/></label><label>Mật khẩu<input type="password" required autoComplete="current-password" value={login.password} onChange={e=>setLogin({...login,password:e.target.value})}/></label>{error&&<div className="suite-login-error">{error}</div>}<button className="suite-btn primary wide" disabled={busy}>{busy?"Đang xác thực...":"Đăng nhập"}</button><small>Giám đốc · Admin · Quản lý · Marketing · Sale · Kế toán</small><PwaInstallCard compact/></form></section></main>}
