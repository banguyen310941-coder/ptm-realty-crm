...
export async function POST(request) {
  try {
    ...
    if (!token) {
      return Response.json({ ok:false,error:"UNAUTHENTICATED" }, {
    ...
    const facebookRetry = result?.ok
      ? await runFacebookRetryBatch({ limit:5,minIntervalMs:60000 }).catch((error) => ({
          ok:false,error:error?.message || "FACEBOOK_RETRY_FAILED"
        }))
      : { ok:true,skipped:true,reason:"SWEEP_NOT_OK" };

    return Response.json(
      { ...(result || { ok:false,error:"AUTOMATION_SWEEP_FAILED" }),facebook_retry:facebookRetry },
      { status:result?.ok ? 200 : 400,headers:{ "Cache-Control":"no-store" } }
    );
  } catch (error) {
    return Response.json({ ok:false,error:error?.message || "AUTOMATION_SWEEP_FAILED" }, {
