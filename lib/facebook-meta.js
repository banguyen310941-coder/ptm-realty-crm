...
const META_PROFILE_TIMEOUT_MS = 7000;
...
export async function fetchMetaProfile(pageId, psid) {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      cache:"no-store",
      signal:AbortSignal.timeout(META_PROFILE_TIMEOUT_MS)
    });
...
export async function sendMetaText(pageId, psid, text) {
  ...
  try {
    ...
    const error = new Error(cause?.name === "TimeoutError" ? "Meta Send API quá thời gian phản hồi." : "Không kết nối được Meta Send API.");
    error.code = cause?.name === "TimeoutError" ? "META_SEND_TIMEOUT" : "META_SEND_NETWORK_FAILED";
    error.cause = cause;
    throw error;
  }
