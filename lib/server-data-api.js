...
const TRANSIENT_RPC_ERROR = /PGRST202|schema cache|could not find the function|fetch failed|network|connection|temporar/i;
...
export async function serverRpc(name,args = {}) {
  ...
  for (let attempt=0; attempt<3; attempt += 1) {
    try {
      ...
      if (TRANSIENT_RPC_ERROR.test(message) && attempt<2) {
        await sleep(attempt === 0 ? 150 : 400);
        continue;
      }
      break;
