...
export const DB_URL = process.env.NEXT_PUBLIC_NEON_DATA_API_URL || DEFAULT_DB_URL;
...
export async function financeRpc(token, action, payload = {}) {
  const response = await client.rpc("crm_finance_api_v2", { p_token: token || "", p_action: action, p_payload: payload });
  if (response.error) {
